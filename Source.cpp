...
void Source::init()
{
	...
	Time to = 1000*1000*x_to(0.1 * 0.001 * 0.001);
	Time tc = 1000*1000*x_tc(0.1 * 0.001 * 0.001);
...
bool Source::run(Time time, MStore *molecules, Obstacle *space)
{
	if (state == State::OPEN)
	{
		...
		{
			state = State::CLOSED;
			open_time = time + 1000*1000*Generator::x_tc_plus40mV(concentration);
			cycle_start_time = closed_time = 1000*1000*Generator::x_to_plus40mV(concentration);
